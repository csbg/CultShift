...
gseadir<-"Ar_07_ORA_from_ClusterProfiler/"
...
combined_invivo[which(combined_invivo['exvivo'] <= -1 & 
                        combined_invivo['invivo'] >= 1.0 ),"group"] <- "g:inv_up.ex_down"
combined_invivo[which(combined_invivo['invivo'] <= -1 & 
                        combined_invivo['exvivo'] >= 1.0 ),"group"] <- "f:inv_down.ex_up"
combined_invivo[which(combined_invivo['invivo'] >= -1 & combined_invivo['invivo'] <=1 &
                        combined_invivo['exvivo'] >= 1.0 ),"group"] <- "e:inv_low.ex_up"
combined_invivo[which(combined_invivo['invivo'] >= -1 & combined_invivo['invivo'] <=1 &
                        combined_invivo['exvivo'] <= -1.0 ),"group"] <- "d:inv_low.ex_down"
combined_invivo[which(combined_invivo['exvivo'] >= -1 & combined_invivo['exvivo'] <=1 &
                        combined_invivo['invivo'] >= 1.0 ),"group"] <- "c:ex_low.in_up"
combined_invivo[which(combined_invivo['exvivo'] >= -1 & combined_invivo['exvivo'] <=1 & 
                        combined_invivo['invivo'] <= -1.0 ),"group"] <- "b:ex_low.in_down"
  ...
  for (KO in unique(combined_invivo$guide)){
    for (type in unique(combined_invivo$group)){
      if (length(combined_invivo[combined_invivo$guide==KO & combined_invivo$group==type,]$geneid) == 0) {
        ...
        enr.res <- enrichr(genes,databases = c("MSigDB_Hallmark_2020", "GO_Biological_Process_2021"))
        ...
        enr.res.list[[KO]] <- enr.res
      }
    }}
 
  enr.res.all <- bind_rows(enr.res.list, .id="KO")
