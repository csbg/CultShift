...
base.dir <- "Ag_ScRNA_01_Trajectories_comparison_30_01_25/"
out <- dirout(base.dir)

InDir <- dirout("SCRNA_10_collect_UMAPs")
ann <- fread(InDir1("invivo_Annotations.tsv"))
...
(base::load(PATHS$SCRNA$MONOCLE.DIR("in.vivo/soupx")))
...
unique(ann$celltype_projection)
...
pDT <- merge(pDT, ann[celltype_projection != "HSC"], by="rn")[!is.na(mixscape_class.global)]
...
names(ff1) <- gsub("OutputCr(.+).tsv", "\\1", ff1)
