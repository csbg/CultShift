...
inDir3<-dirout_load("Ag_ScRNA_09_Normalized_counts_ex_in_izzo_NTC_guide")
base<-"Ag_ScRNA_14_per_celltype_gene_plots_guide/"
basedir<-dirout("Ag_ScRNA_14_per_celltype_gene_plots_guide")
########################################################################
#load limma results from NTC
dataVoom_NTC<-read_rds(inDir1("dataVoom_perCTex.vivovsin.vivo.rds"))
dataVoom_all_NTC<-read_rds(inDir3("dataVoom_perCT_NTC_izzo_ex_in.rds"))
...
###########################################################################
meta_all_NTC<-read_rds(inDir3("meta_all_NTC.rds"))
###########################################################################
...
############################################
dir<-dirout(paste0(base,"NTC",gsub(" ","_",terms_of_interest)))
...
ggsave(dir(paste0(gsub(" ","_",terms_of_interest))), 
       w=8,h=10)
#unfiltered
...
############################
# Iterate over each unique gene
IFN<-unique(IFN_filtered_p_value$ensg)
dat.list<-list()
for(gg in IFN) {
  # Subset the metadata and E values for the current gene
  gene_data <- meta_all_NTC %>%
    mutate(E = scale(dataVoom_all_NTC$E[gg,])) %>%
    rownames_to_column("sample1") %>%
    remove_rownames()%>%
    mutate(gene=gg)
