...
out <- dirout(base.dir)
inp<-dirout("SCRNA_33_DE_Nebula_testClustering/")
InDir <- dirout("SCRNA_10_collect_UMAPs")
ann <- read_rds(InDir("/ProjVivo_celltypes.RDS"))



celltypes <- fread('metadata/FIGS_celltypes.tsv')

celltypes <- celltypes[Name %in% ann$functional.cluster]
...
for(typex in unique(celltypes[Type != "HSC"]$Type)){
  print(typex)
  cells <- ann[functional.cluster %in% celltypes[Type %in% c(typex, "HSC")]$Name]$rn
...
pDT <- merge(pDT, ann[functional.cluster != "HSC"], by="rn")[!is.na(mixscape_class.global)]
