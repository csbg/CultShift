...
(fish.test.x <- names(fish.test.sets)[2])
foreach(fish.test.x = names(fish.test.sets)) %dopar% {
  ...
  for(sx in unique(pDT1$timepoint)){
    ...
    for(gx in unique(pDT2[mixscape_class != "NTC"]$genotype)){
      ...
      for(cx in unique(pDT2$Clusters)){
        ntc <- unique(pDT2[mixscape_class == "NTC"][,.N, by="CRISPR_Cellranger"][N > 20]$CRISPR_Cellranger)[2]
  ...
  # plot
  res <- hierarch.ordering(res, toOrder = "grp", 
                           orderBy = "Clusters",
                           value.var = "log2OR", 
                           aggregate = TRUE)
  ggplot(res, aes(
