...
performDE <- function(meta, counts, tissue_type1, tissue_type2) {
  ...
  d <- d0[-drop,] 
  # Example gene filtering
  keep <- filterByExpr(d)
  d <- d[keep, ]
  
  
  meta$tissue <- factor(meta$tissue, levels=c(tissue_type2, tissue_type1))
  ...
  for (comp_list in comparisons) {
    ...
    print(paste("Contrast Matrix for", id, ":", contrast))
    ...
    # Debugging: Print contrast variable before calling contrasts.fit()
    print(contrast)
    ...
    # Debugging: Print top table results
    print(paste("Top Table Results for", id, ":"))
    print(head(top_table[[id]]))
  }
  ...
  #out <- dirout(paste0(basedir, "per_cell_type/"))
  
  top_table_res %>% write_rds(basedir("limma_perCTex.vivovsin.vivo.rds"))
