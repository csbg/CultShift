...
deg_plot_data <- as.data.frame(deg_plot_data)# Prepare DEG data
...
Fig2.2
Fig2.2_legend <- correlation_deg_flagged %>%
  ggplot() +
  geom_point(aes(
    x = genotype,
    y = celltype,
  ...
  optimized_theme_fig()+
  theme(legend.position = "right")
ggsave(basedir("Fig2.2_legend.pdf"),Fig2.2_legend,
       w=17,
       h=7, units = "cm")

#
#Fig2.3 DEG nteraction logFC ---------------------------------------------
#
...
Fig2.3
ggsave(basedir("Fig2.3.pdf"),plot=Fig2.3_legend,w=5,h=4, units = "cm")
Fig2.3_legend <- summary_df %>%
  filter(Count > 10, coef %in% koi) %>%
  # Plot with log10 scale on y-axis
  ggplot(aes(x = coef, y = Regulation, color = Regulation)) +
  geom_point(aes(size=log10(Count))) +    
...
ggsave(basedir(paste0("Fig2.3.1_supplementary.pdf")),plot = Fig2.3.1_supplementary, 
       width = 11*0.58*3,
       height = length(unique(filtered_genes$ensg))*0.13*2)
...
pathway_list <- list(
  ...
  "Myc_Targets" = enr.terms$MSigDB_Hallmark_2020$`Myc Targets V2`,
  "ROS" = enr.terms$MSigDB_Hallmark_2020$`Reactive Oxygen Species Pathway`,
  "E2F_targets" = enr.terms$MSigDB_Hallmark_2020$`E2F Targets`,
...
# Plot the results
Fig2.3.1_supplementary_supp <- ggplot(combined_results, aes(x = coef, y = ensg,
  ...
  scale_size_continuous(
    range = c(0, 6),  # Set size range
    breaks = c( 1,2, 4, 5)  # Size breaks
  ) +
  labs(title = "Differentially expressed genesets",
       x = "KOs",
       y = "Genes",
  ...
  theme_bw() +
  optimized_theme_fig()
Fig2.3.1_supplementary_supp

# Save the plot
ggsave(basedir(paste0("Fig2.3.1_supplementary_supp.pdf")), plot = Fig2.3.1_supplementary_supp,
       width = 11 * 0.8 *2,
       height = length(unique(combined_results$ensg)) * 0.18 )
