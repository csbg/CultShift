...
process_with_soupx <- function(seurat.obj, filtered.gx, raw.gx,dsx = dsx, 
                               interferon_genes = interferon_genes, 
                               cholesterol_genes = cholesterol_genes, 
                               hemoglobin_genes_mouse = hemoglobin_genes_mouse) {
  ...
  write_tsv(total_contribution_df,out(paste0(dsx,"totalsoup.tsv")))
...
for(dsx in unique(SANN[tissue %in% c("in.vivo","ex.vivo","leukemia"),]$sample_found)) {
