...
corr <- "SCRNA_33_DE_Nebula_testClustering/Output/exvivo_9dinvivo14d/"

out_corr <- dirout(corr)

correlation_logfc_in_ex<-fread(out_corr("correlation_logfc_invivo_14dvs_exvivo9d.tsv"))

colnames(correlation_logfc_in_ex)<-c("gene","correlation")


# Load annotation ---------------------------------------------------------
(base::load(PATHS$SCRNA$MONOCLE.DIR("in.vivo")))
