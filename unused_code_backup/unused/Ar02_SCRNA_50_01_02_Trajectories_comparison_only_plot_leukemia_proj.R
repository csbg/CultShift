...
basedir_corr <- "Ag_logfc_correlation//exvivo_9dinvivo14d/"
out_corr <- dirout(basedir_corr)
correlation_logfc<-fread(out_corr("correlation_logfc_invivo_14dvs_exvivo9d_pearson_.tsv"))
...
ff1 <- ff1[!grepl("ex.vivo", ff1)]
names(ff1) <- gsub("Output_(.+).tsv", "\\1", ff1)

pDT_leuk <- rbindlist(lapply(ff1, function(fx) fread(out(fx))), idcol = "sample")
...
pDT.sum_ex[gene=="NTC"]
pDT.sum_leuk_combined<-rbind(pDT.sum_leuk[gene %in% pDT.distr_ex$gene],pDT.sum_ex)
pDT.sum__leuk_combined_Mye<-pDT.sum_combined[celltype=="Mye"]

Mye_leuk<-pDT.distr_ex[celltype=="Mye"][,c("gene","rn","UMAP1","UMAP2","traj","sample.x","celltype","tissue","timepoint","type","traj.scale")]
...
Mye_leuk<-rbind(Mye_leuk,pDT.distr_in)
...
col<-merge(pDT_summary_Mye,correlation_logfc,by="gene")
col[,correlation_logfc:= correlation]
...
pDT.ntc_sum_plot_color<-merge(pDT.ntc_sum_plot,correlation_logfc,by="gene")
