...
basedir<-"Ag_ScRNA_09_Pseudobulk_limma_all/"
enrichment<-dirout("Ag_ScRNA_09_Pseudobulk_limma_all_enrichment")
##########################################################
...
performDE <- function(meta, counts, model_formula) {
  ...
  pmap(unique(limmaRes$coef),~{
    ...
    out <- dirout(paste0(basedir, coefx))
...
####################################
#all celltypes exvivo effect for ntc
####################################
...
map(unique(meta$celltype), ~ {
  ...
  if(nrow(meta_ct[meta_ct$tissue=="in.vivo",])>1 &
    ...
    combined_results$group <- ifelse(combined_results$logFC >= 1 &
                                       combined_results$adj.P.Val <= 0.05, "up",
                                     ifelse(combined_results$logFC <= -1 &
                                              combined_results$adj.P.Val <= 0.05, "down", "n.s"))
  }else{
    print("null")
  }
})
...
process_celltype <- function(ct) {
  ...
  enr.res_up <- enrichr(goi_up, databases = c("KEGG_2019_Mouse" ,
  ...
  enr.res_down <- enrichr(goi_down, databases = c("KEGG_2019_Mouse",
                                                  "MSigDB_Hallmark_2020",
                                                  "WikiPathways_2019_Mouse",
                                                  "GO_Biological_Process_2021")) %>%
    bind_rows(.id = "db") %>%
