...
for (ex in c("7d","9d")){
  for (inv in c("14d","28d")){
    ...
    out1 <- dirout(paste0(basedir,"/exvivo_",ex,"leukemia",leuk))
    ...
    for (corr in c("pearson","spearman")){
      ...
      test<-cor(combined_ex_le[grep("exvivo",colnames(combined_ex_le))],y=combined_ex_le[grep("leuk",colnames(combined_ex_le))],
                method = corr)
      ...
      write.tsv(data.table(data.frame(diag_correlation), keep.rownames = TRUE),
                out1(paste0("correlation_logfc_leuk_",leuk,"vs_exvivo",ex,".tsv")))
      ...
      ggsave(out1("pheatmap_corresponding_ko","leuk",leuk,"exvivo",ex,".png"), w=15,h=15)
