...
ko_flags <- meta %>%
  dplyr::group_by(genotype, celltype, tissue) %>%
  dplyr::summarise(num_samples = n_distinct(sample1), .groups = "drop") %>%
  tidyr::pivot_wider(
    names_from = tissue,
    values_from = num_samples,
    values_fill = 0
  ) %>%
  dplyr::mutate(valid_ko = (in.vivo >= 3 & ex.vivo >= 3)) %>%
  dplyr::group_by(genotype, celltype) %>%
  dplyr::summarise(valid_ko = any(valid_ko), .groups = "drop") %>%
  dplyr::mutate(coef = genotype)

#--- Compute replicate counts and valid_ko per KO ---#
replicates_per_ko <- meta %>%
  group_by(genotype, celltype, tissue) %>%
  dplyr::summarize(num_samples = n_distinct(sample1), .groups = 'drop') %>%
...
selected_KOs <- meta %>%
  ...
  dplyr::summarize(num_sample = n_distinct(sample1), .groups = 'drop') %>%
...
valid_ko_summary <- ko_flags %>%
  group_by(genotype) %>%
  dplyr::summarize(has_valid_celltype = any(valid_ko), .groups = 'drop')
###############
