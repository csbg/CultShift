...
library(monocle3)
out <- dirout("/Ag_ScRNA_17_proj_celltype_in_monocle_obj_NO_SOUPX")
inDir1<- dirout_load("Ag_SCRNA_05_01_UMAPs_and_celltypes/")
inDir <- dirout_load("/media/AGFORTELNY/PROJECTS/TfCf/Analysis/SCRNA_02_01_Integration/")
...
tissues <- c("ex.vivo", "in.vivo")

##################
mobjs <- lapply(tissues, function(tissuex) {
  tryCatch({
    # Construct file path
    file_path <- paste0("/media/AGFORTELNY/PROJECTS/TfCf/Analysis/SCRNA_02_01_Integration/",tissuex,"/MonocleObject.RData")
