optimized_theme_fig <- function() {
  theme_bw() +  # Start with a clean, white background theme
    theme(
      ...
      axis.text = element_text(size = 6, color = "black"),                              # Clear axis text with larger size
      axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 6, color = "black"),  # Angled X-axis labels for better readability
      
      # Legend
      legend.title = element_text(size = 6, face = "bold"),            # Bold legend title
