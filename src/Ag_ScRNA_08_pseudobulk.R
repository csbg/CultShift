...
#paths------
#############
out <- dirout("/Ag_ScRNA_08_pseudobulk")
...
process_monocle_data <- function(monocle_obj, annotations, tissue){
  ...
  #######
  data<-as.data.frame(monocle_obj@colData)
  celltype_samp <- data %>%
    group_by(tissue,sample,celltype,genotype) %>%
  ...
  return(list(result = result, meta = meta,data=data))
}
...
data_ex<-Output_ex$data
write.csv(df_counts_ex,out("ex_counts.csv"))
write.csv(meta_ex, file = out("metadata_ex.csv"), row.names = T)
write.csv(data_ex,file = out("data_ex.csv"), row.names = T)
...
data_in<-Output_in$data
write.csv(df_counts_in,out("in_counts.csv"))
# Merge and write combined counts
df_counts <- merge(df_counts_ex, df_counts_in, by = "row.names")
writectsv(df_counts,out("in_counts.csv"))
write.csv(meta_in, file = out("metadata_in.csv"), row.names = T)
write.csv(data_in,file = out("data_in.csv"), row.names = T)
...
write.csv(combined_meta, file = out("metadata.csv"), row.names = T)
