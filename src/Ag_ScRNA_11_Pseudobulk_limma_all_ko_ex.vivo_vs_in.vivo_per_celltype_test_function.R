...
performDE <- function(meta, counts, model_formula, output_dir = "dataVoom_plots/") {
  ...
  for (ct in unique_celltypes) {
    ...
    meta_subset <- meta_subset %>% mutate(across(where(is.factor), droplevels))
    
    # Check if tissue and genotype have at least two levels
    if (!("tissue" %in% colnames(meta_subset)) || 
        !("genotype" %in% colnames(meta_subset)) || 
        length(unique(meta_subset$tissue)) < 2 || 
        length(unique(meta_subset$genotype)) < 2) {
      cat("Skipping DE analysis for cell type:", ct, "because tissue or genotype has only one level or is missing.\n")
      next
    }
    ...
    non_estimable_coefs <-  nonEstimable(modelMatrix)
