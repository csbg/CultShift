...
for(dsx in SANN$sample_found){
  ...
  # SoupX Analysis
  if("CRISPR.Guide.Capture" %in% names(additional.info.x)){
    guide_matrix <- additional.info.x[["CRISPR.Guide.Capture"]]
    # Assume soup_x_data is a function that processes your matrix correctly
    soupX_obj <- SoupX(counts = data.gx, guides = guide_matrix)
    seurat.obj <- UpdateSeuratObject(seurat.obj, counts = soupX_obj$corrected_counts)
  }
