...
out <- dirout("Ag_external_datasets_generalized_modifying_colors")
...
selected_sets <- c("ISGs", "ISG_core", "mTORC1", "Housekeeping", "Cholesterol", "ISGs_glioblastoma")
...
all_zscore_data <- all_zscore_data %>%
  mutate(
    ...
    geneset = factor(geneset, levels = c("ISGs", "ISG_core", "mTORC1", "Cholesterol", "Housekeeping","ISGs_glioblastoma"))
  )
...
geneset_levels <- c("ISGs", "ISG_core", "mTORC1", "Housekeeping", "Cholesterol","ISGs_glioblastoma")
