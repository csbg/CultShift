...
(tx <- SANN$tissue[20])
for(tx in unique(SANN$tissue)[1]){
  ...
  for(sx in SANN[tissue == tx]$sample){
    ...
    print(paste("Trying to load:", fx))
    
    if(!file.exists(fx)) stop(fx, " seurat object not found")
    
    # Load the file and check if seurat.obj exists
    tryCatch({
      print(paste("Reading", sx))
      base::load(fx)
      print("File loaded successfully")
    }, error = function(e) {
      print(paste("Failed to load file:", fx))
      print(e)
    })
    
    if(!exists("seurat.obj") || is.null(seurat.obj)) {
      stop("Seurat object not found or is NULL in the file: ", fx)
    }
    
    # Now safely access seurat.obj@meta.data
    print(head(seurat.obj@meta.data))  # Print meta.data for debugging
    ...
    monocle.obj.list[[sx]] <- new_cell_data_set(expression_data = mat.use,
    ...
    if(sx == "DM_CITEseq-2_NA_NM_1"){
      ...
      save(citeseq.MT,
           file=out.base("CITESEQ_Antibodies.RData"))
    }
  }
...
#
#################################################
for(tx in unique(SANN$tissue)[2:3]){
  out <- dirout(paste0(basedir, tx, "/soupx/"))
  monocle.file <- out("MonocleObject.RData")
