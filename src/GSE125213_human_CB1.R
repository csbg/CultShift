...
# check that Ifit1 exists in your matrix
"GBP2" %in% rownames(dataVoom$E)

# extract Ifit1 expression
GBP2_expr <- dataVoom$E["GBP2", ]

# simple barplot
barplot(GBP2_expr,
        las = 2,                      # rotate sample names
        col = "steelblue",
        main = "GBP2 Expression Across Samples",
        ylab = "log2 CPM (voom normalized)")
#########################
# check that Ifit1 exists in your matrix
"IRF7" %in% rownames(dataVoom$E)

# extract Ifit1 expression
IRF7_expr <- dataVoom$E["IRF7", ]

# simple barplot
barplot(IRF7_expr,
        las = 2,                      # rotate sample names
        col = "steelblue",
        main = "IRF7 Expression Across Samples",
        ylab = "log2 CPM (voom normalized)")

# check that idi1 exists in your matrix
"IDI1" %in% rownames(dataVoom$E)

# extract Ifit1 expression
IDI1_expr <- dataVoom$E["IDI1", ]

# simple barplot
barplot(IDI1_expr,
        las = 2,                      # rotate sample names
        col = "steelblue",
        main = "IDI1 Expression Across Samples",
        ylab = "log2 CPM (voom normalized)")

# check that idi1 exists in your matrix
"SQLE" %in% rownames(dataVoom$E)

# extract Ifit1 expression
SQLE_expr <- dataVoom$E["SQLE", ]

# simple barplot
barplot(SQLE_expr,
        las = 2,                      # rotate sample names
        col = "steelblue",
        main = "SQLE Expression Across Samples",
        ylab = "log2 CPM (voom normalized)")
