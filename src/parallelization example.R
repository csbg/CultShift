...
#function
################################
process_with_soupx <- function(seurat.obj, filtered.data, raw.data,dsx) {
  # Check and create SoupChannel object
  sc <- tryCatch({
    SoupChannel(raw.data, filtered.data, exprsType = "UMI")
  }, error = function(e) {
    stop("Error creating SoupChannel object: ", e$message)
  })
  
  # Create and process Seurat object
  seurat.obj <- CreateSeuratObject(counts = filtered.data)
  ...
  autoEstCont(sc, tfidfMin = 0.8, forceAccept = TRUE)
  dev.off()
  
  # Calculate and save gene set contributions
  contributions <- list(
    hb = data.frame(Gene = rownames(sc$soupProfile[rownames(sc$soupProfile) %in% hemoglobin_genes_mouse, ]),
                    sc$soupProfile[rownames(sc$soupProfile) %in% hemoglobin_genes_mouse, ],
                    stringsAsFactors = FALSE),
    ifn = data.frame(Gene = rownames(sc$soupProfile[rownames(sc$soupProfile) %in% interferon_genes, ]),
                     sc$soupProfile[rownames(sc$soupProfile) %in% interferon_genes, ],
                     stringsAsFactors = FALSE),
    chol = data.frame(Gene = rownames(sc$soupProfile[rownames(sc$soupProfile) %in% cholesterol_genes, ]),
                      sc$soupProfile[rownames(sc$soupProfile) %in% cholesterol_genes, ],
                      stringsAsFactors = FALSE)
  )
  ...
  # Save the contributions with rownames included as a "Gene" column
  #write.table(contributions$hb, file = out(paste0(dsx, "soup_cont.hb.tsv")), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  #write.table(contributions$ifn, file = out(paste0(dsx, "soup_cont.IFN.tsv")), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  #write.table(contributions$chol, file = out(paste0(dsx, "soup_cont.chol.tsv")), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  
  # Save the total contribution
  #write.table(data.frame(Sample = dsx, Total_Contribution = total_contribution), file = out(paste0(dsx, "totalsoup.tsv")), sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
...
# Define your function to process a single sample
process_sample <- function(dsx) {
  # Ensure that dsx is correctly passed and used
  ...
  # SoupX
  results <- process_with_soupx(seurat.obj, filtered.gx, raw.gx)
  seurat.obj <- results$seurat_obj 
  sc <- results$contamination_plot
  contributions <- results$ontributions
  ...
  #
  # Save the contributions with rownames included as a "Gene" column
  write.table(contributions$hb, file = out(paste0(dsx, "soup_cont.hb.tsv")),
              sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  write.table(contributions$ifn, file = out(paste0(dsx, "soup_cont.IFN.tsv")), 
              sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  write.table(contributions$chol, file = out(paste0(dsx, "soup_cont.chol.tsv")), 
              sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
  
  # Save the total contribution
  write.table(data.frame(Sample = dsx, Total_Contribution = total_contribution), 
              file = out(paste0(dsx, "totalsoup.tsv")), sep = "\t", 
              quote = FALSE, row.names = FALSE, col.names = TRUE)
  seurat.obj[["percent.mt"]] <- PercentageFeatureSet(seurat.obj, pattern = "^mt-")
  seurat.obj$md5sum <- dsx.md5sum
  
  # Set cutoffs and plots
  cutoffs <- list(
  ...
  for(qcx in names(cutoffs)) {
    ggplot(data.frame(Value=seurat.obj@meta.data[[qcx]]),
           aes(x=Value)) + stat_density() + scale_x_log10() +
      theme_bw(12) +
  ...
  # Subset and process
  seurat.obj <- subset(seurat.obj, subset = nFeature_RNA > cutoffs$nFeature_RNA &
                         nCount_RNA > cutoffs$nCount_RNA & 
                         percent.mt < cutoffs$percent.mt)
  seurat.obj <- NormalizeData(seurat.obj, verbose = FALSE)
  seurat.obj <- CellCycleScoring(seurat.obj, s.features = cc.genes$s.genes,
  ...
  for(tx in setdiff(names(additional.info.x), "CRISPR_Cellranger")) {
    ...
    if(txn == "CRISPR.Guide.Capture") {
      ...
      cutoff <- 5
      x <- as.matrix(x[,apply(x, 2, max) >= cutoff, drop=F])
      ii <- apply(x, 2, function(col) which(col >= cutoff))
    }
  ...
  if("CRISPR_Cellranger" %in% names(additional.info.x)) {
    ...
    if(any(grepl("\\|", x$feature_call))) {
      x$top.guide <- as.numeric(sapply(strsplit(x$num_umis, "\\|"), function(i) {
        i <- as.numeric(i)
        i <- which(i/sum(i) > 0.75)
        if(length(i) == 0) NA else paste(i, collapse = "|")
      }))
    ...
    seurat.obj@meta.data[["CRISPR_Cellranger"]] <- setNames(x$guide,
                     x$cell_barcode)[row.names(seurat.obj@meta.data)]
  }
  
  # Mixscape
  guides.use <- intersect(c("CRISPR_Cellranger", "CRISPR.Guide.Capture"),
                          colnames(seurat.obj@meta.data))[1]
  if(!is.na(guides.use)) {
    ...
    if(sum(!is.na(guides.clean)) > 5 & "NTC" %in% guides.clean) {
      ...
      eccite <- subset(eccite, features=names(which(Matrix::rowSums(eccite@assays$RNA@counts) >= 100)))
      eccite <- FindVariableFeatures(eccite, selection.method = "vst", nfeatures = 2000)
      eccite <- ScaleData(eccite, verbose = FALSE)
      eccite <- RunPCA(eccite, verbose = FALSE)
      eccite <- RunUMAP(eccite, dims = 1:30, verbose = FALSE)
      
      p <- DimPlot(eccite, reduction = "umap", group.by = "guide") + theme_bw() + ggtitle(paste(dsx, "Mixscape"))
      ggsave(out("Mixscape_", dsx, ".pdf"), p, width = 10, height = 8)
    }
  }
  ...
  return(list(
    seurat_obj = seurat.obj,             # Updated Seurat object
    contamination_plot = sc,             # Contamination plot (SoupChannel object)
    contributions = contributions,       # Contributions data
    total_contribution = total_contribution )) # Total contribution
}


# Define the number of cores to use
numCores <- min(5, detectCores())  # Use a maximum of 5 cores
...
samples <- unique(SANN[tissue %in% c("in.vivo", "ex.vivo", "leukemia"), ]$sample_found)[24:28]

# Collect all results in the parallel loop
results_list <- foreach(dsx = samples, 
                        .packages = c("Seurat", "Matrix", "data.table", "ggplot2", "SoupX"),
                        .options.multicore = list(preschedule = FALSE, set.seed = TRUE),
                        .maxcombine = 7) %dopar% {
                          result <- process_sample(dsx)
                          return(list(dsx = dsx, result = result))
                        }

# Write files sequentially after parallel processing
for (res in results_list) {
  dsx <- res$dsx
  result <- res$result
  sc <- result$contamination_plot
  
  # Save files (ensure unique filenames if needed)
  pdf(out(paste0(dsx, "_SOUPX_plot.pdf")))
  autoEstCont(sc, tfidfMin = 0.8, forceAccept = TRUE)
  dev.off()
  
  # Save other files (contributions, etc.)
  # ...
}
results_list <- list()
for(dsx in samples) {
  result <- process_sample(dsx)
  results_list[[dsx]] <- result
}
results_list <- foreach(dsx = samples, 
                        .packages = c("Seurat", "Matrix", "data.table", "ggplot2", "SoupX"),
                        .options.multicore = list(preschedule = FALSE, set.seed = TRUE),
                        .errorhandling = "pass",  # Continue on error, but capture the error message
                        .maxcombine = 7) %dopar% {
                          tryCatch({
                            result <- process_sample(dsx)
                            list(dsx = dsx, result = result)
                          }, error = function(e) {
                            list(dsx = dsx, error = e$message)
                          })
                        }
results_list <- foreach(dsx = samples, 
                        .packages = c("Seurat", "Matrix", "data.table", "ggplot2", "SoupX"),
                        ...
                        .errorhandling = "stop") %dopar% {
                          tryCatch({
                            # Process the sample and return results
                            result <- process_sample(dsx)
                            return(list(dsx = dsx, result = result))
                          }, error = function(e) {
                            message(paste("Error processing sample:", dsx))
                            stop(e)  # Stop the loop when an error is encountered
                          })
                        }
...
num_cores <- min(parallel::detectCores() - 1, 5)

# Set the future plan to use multisession with the specified number of cores
plan(multisession, workers = num_cores)
results <- future_map(samples, process_sample)
library(foreach)
library(doParallel)
library(doRNG) # Optional: for reproducibility
results <- foreach(dsx = unique(SANN[tissue %in% c("in.vivo", "ex.vivo", "leukemia"),]$sample_found)[24:55], 
                   .packages = c("Seurat", "Matrix", "data.table", "ggplot2", "SoupX"), 
                   .export = c("process_with_soupx", "SCRNA.DietSeurat", "cc.genes", "hemoglobin_genes_mouse", "interferon_genes", "cholesterol_genes")) %dopar% {
                     ...
                     if(!is.na(guides.use)) {
                       ...
                       if(sum(!is.na(guides.clean)) > 5 & "NTC" %in% guides.clean) {
                         ...
                         seurat.obj <- Seurat::RunPCA(seurat.obj, features = VariableFeatures(seurat.obj), npcs = 30, verbose = FALSE)
                         seurat.obj <- Seurat::RunTSNE(seurat.obj, dims = 1:30, verbose = FALSE)
                         seurat.obj <- Seurat::RunUMAP(seurat.obj, dims = 1:30, verbose = FALSE)
                         seurat.obj <- Seurat::FindNeighbors(seurat.obj, dims = 1:30, verbose = FALSE)
                         seurat.obj <- Seurat::FindClusters(seurat.obj, verbose = FALSE)
                         
                         p1 <- Seurat::DimPlot(seurat.obj, group.by = "guide", label = TRUE) + ggtitle("Guide")
                         p2 <- Seurat::DimPlot(seurat.obj, group.by = "seurat_clusters", label = TRUE) + ggtitle("Cluster")
                         p3 <- Seurat::DimPlot(seurat.obj, group.by = "sample", label = TRUE) + ggtitle("Sample")
                         
                         ggsave(out("DimPlot_", dsx, ".pdf"), p1, width = 8, height = 8)
                         ggsave(out("DimPlot_Cluster_", dsx, ".pdf"), p2, width = 8, height = 8)
                         ggsave(out("DimPlot_Sample_", dsx, ".pdf"), p3, width = 8, height = 8)
                       } else {
                         message("Not enough data or 'NTC' not present for ", dsx)
                       }
                     }
