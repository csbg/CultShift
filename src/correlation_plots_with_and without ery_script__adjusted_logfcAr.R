...
for (ex in c("7d","9d")){
  for (inv in c("14d","28d")){
    ...
    exvivo[,est.q_val:=sign(estimate)*q_value]
    ...
    invivo = fread(dirout(paste0(inp, "in.vivo_",inv,"_useClusters"))("DEG_Results_all.tsv")) %>% 
      select(c("guide","gene_id","estimate","q_value"))
    invivo[,est.q_val:=sign(estimate)*q_value]
    ...
    leukemia=fread(dirout(paste0(inp, "leukemia_",leuk,"_useClusters"))("DEG_Results_all.tsv")) %>% 
      select(c("guide","gene_id","estimate","q_value"))
    leukemia[,est.q_val:=sign(estimate)*q_value]
    ...
    combined<-merge(exvivo,invivo,by=c("guide","gene_id")) %>% pivot_wider(
      names_from = guide,
      values_from=c(est.q_val.x,est.q_val.y)
    )
    colnames(combined)[grep("est.q_val.x",colnames(combined))]<-gsub("est.q_val.x","exvivo",colnames(combined)[grep("est.q_val.x",colnames(combined))])
    colnames(combined)[grep("est.q_val.y",colnames(combined))]<-gsub("est.q_val.y","invivo",colnames(combined)[grep("est.q_val.y",colnames(combined))])
    ...
    out1 <- dirout(paste0(basedir,"/exvivo_",ex,"invivo",inv))
      for (corr in c("pearson","spearman")){
        ...
        png(out1(paste0("all_correlations_",corr,"_.png")))
