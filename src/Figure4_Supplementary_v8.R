...
out <- dirout("Figure4_Supplementary_v8")
...
selected_sets <- c("ISGs_glioblastoma")
...
all_zscore_data <- all_zscore_data %>%
  mutate(
    ...
    geneset = factor(geneset, levels = c("ISGs_glioblastoma"))
  )
...
geneset_levels <- c("ISGs_glioblastoma")
