...
base <- "Ag_ScRNA_09_pseudobulk_per_celltype_limma_NTC_guide_exvivo2/"
basedir <- dirout("Ag_ScRNA_09_pseudobulk_per_celltype_limma_NTC_guide_exvivo2/")
source("src/Ag_Optimized_theme_fig.R")
##################################################################################
#load data
########################
#metadata
counts_ex <- readRDS(inDir("ex.vivo_counts.rds"))
counts_in <- readRDS(inDir("in.vivo_counts.rds"))

#meta data
exvivo_meta <- readRDS(inDir("ex.vivo_meta.rds"))
invivo_meta <- readRDS(inDir("in.vivo_meta.rds"))
correct_celltype_names <- function(names_vec) {
  ...
  names_vec <- gsub("Early GMP", "GMP.early", names_vec)
  names_vec <- gsub("Gran Prog.", "Gran.P", names_vec)
  names_vec <- gsub("Late GMP", "GMP.Late", names_vec)
  names_vec <- gsub("Ery \\(G1\\)", "Ery.G1", names_vec)
  names_vec <- gsub("Ery \\(S\\)", "Ery.S", names_vec)
  names_vec <- gsub("Ery Pert.", "Ery.Pert", names_vec)
  names_vec <- gsub("Ery Prog.", "Ery.P", names_vec)
  names_vec <- gsub("Eo/Ba", "Eo.Ba", names_vec)
  
}
rownames(exvivo_meta) <- correct_celltype_names(rownames(exvivo_meta))
rownames(invivo_meta) <- correct_celltype_names(rownames(invivo_meta))
...
#colnames
colnames(counts_ex) <- correct_celltype_names(colnames(counts_ex))
colnames(counts_in) <- correct_celltype_names(colnames(counts_in))
...
meta <- rbind(exvivo_meta, invivo_meta)
meta$celltype <- correct_celltype_names(meta$celltype)
...
NTC_meta <- meta |> filter(genotype == "NTC")|>
  filter(celltype %in% unique(exvivo_meta$celltype))
NTC_meta_filtered <- NTC_meta |>
  group_by(celltype, tissue) |>
  filter(n() >= 3) %>%
  ungroup()|>
  pull(celltype)|> unique()
NTC_meta <- meta |> filter(genotype == "NTC")|>
  filter(celltype %in% NTC_meta_filtered)

NTC_meta %>% write_rds(basedir("NTC_meta.rds"))

counts <- cbind(counts_ex,counts_in)

NTC_counts <- counts[,rownames(NTC_meta)]
write.table(NTC_counts,basedir("NTC_counts.tsv"))
...
#generate gene panels
top_genes <- ex_in_NTC_per_ct[ex_in_NTC_per_ct$genes %in% c("Idi1","Ifit1","Iigp1","Gbp2"),] %>%
  unique()
ggplot() +
