...
out <- dirout("GSE162698_TAM_invitro_invivo")
...
urld <- "https://www.ncbi.nlm.nih.gov/geo/download/?type=rnaseq_counts&acc=GSE162698&format=file&file=GSE162698_raw_counts_GRCh38.p13_NCBI.tsv.gz"
path <- paste(urld, "acc=GSE162698", "file=GSE162698_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&")
...
# Metadata
gse <- getGEO("GSE162698", GSEMatrix = TRUE)
metadata <- gse$GSE162698_series_matrix.txt.gz
...
metadata <- metadata %>% dplyr::select(title, geo_accession, tissue)
