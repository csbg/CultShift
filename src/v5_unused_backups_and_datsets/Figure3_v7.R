...
Fig3A <- correlation_deg_flagged %>%
  ...
  labs(x = "KOs",
       y = "Cell type",
       title = 
         "Corralation reveals discordant KO effects in vivo vs ex vivo KO effect in hematopoietic cells"
  )+
  optimized_theme_fig()+
  theme(
...
Fig3A
ggsave(basedir("Fig3A_alternate.pdf"))
...
##################################################################
#alternate
Fig3C_alternate <- ggplot(deg_plot_data %>% filter(cell_type == "T8"), aes(x = genotype, y = correlation)) +
