...
Fig2.1 <- correlation_deg %>% 
  ...
  scale_fill_gradient2(low = "#4C889C", mid = "white", high = "#D0154E") +  # Use 'fill' for inside color
  labs(x = "KOs",
       y = "Celltype") +
  optimized_theme()
...
pathways <- split(genes_fig_1$ensg, genes_fig_1$pathway)
unique(genes_fig_1$genes)
...
fig2.3_1 <- ggplot(filtered_genes, aes(x = coef, y = ensg,
                                     color = logFC,
  ...
  optimized_theme()
fig2.3_1 
ggsave(basedir(paste0("fig2.3_1.pdf")),plot = fig2.3_1, 
       width = 13*0.31*5,
       height = length(unique(combined$ensg))*0.2+2)
#top_genes_from selected pathways
...
get_top_genes <- function(pathway_name,
  ...
  filtered_genes <- limma_results %>%
    ...
    arrange(adj.P.Val) %>%  # Sort by absolute logFC in descending order
...
filt <-  combined %>% 
    group_by(Geneset,celltype,coef)%>%
    arrange(adj.P.Val)%>%
    slice_head(n=10)%>%
    pull(ensg)%>%
    unique()
  combined <- combined %>% filter(ensg %in% filt)
  len <-length(unique(combined$ensg))
  ggplot(combined,
    ...
    scale_size_continuous(range = c(2, 6), breaks = c(0,1,1.5, 2,4)) +
    
    labs(
      title = paste("ISG core and Cholesterol genes in KOs"),
      x = "Knockouts",
      y = "Genes",
      color = "logFC",
      size = "-log10(adj.P.Val)"
    ) +
    facet_grid(cols = vars(celltype),rows = vars(Geneset))+
    theme_bw() +  # Use a clean theme
    optimized_theme()
    
    ggsave(basedir(paste0("fig2.3", ".pdf")), width = 13*0.35*5, height = len*0.15+2)
...
create_plot <- function(data) {
  # Filter the data and select top 5 per geneset and coef
  filt <- data %>%
    group_by(Geneset, celltype, coef) %>%
    arrange(adj.P.Val) %>%
    slice_head(n = 3) %>%
  ...
  # Filter the data based on selected genes
  data <- data %>% filter(ensg %in% filt)
  
  # Determine number of unique genes
  len <- length(unique(data$ensg))
  
  # Create the plot with facets split by celltype and geneset
  ggplot(data, aes(x = gsub("interaction", "", coef), y = ensg,
                   color = logFC, size = pmin(30, -log10(adj.P.Val)))) +
    geom_point() +  # Scatter plot
    scale_color_gradient2(low = "blue", mid = "white", high = "red") +  # Color scale for logFC
    scale_size_continuous(range = c(2, 6), breaks = c(0, 1, 1.5, 2, 4)) +
    
    labs(
      title = "LogFC and adj.P.Val for Gene Sets",
      x = "Coefficient",
      y = "Genes",
      color = "logFC",
      size = "-log10(adj.P.Val)"
    ) +
    facet_grid( Geneset ~ celltype , scales = "free_y") +  # Facet by celltype and geneset
    theme_bw() +  # Use a clean theme
    optimized_theme()
  
  # Save the plot
  ggsave(basedir("all_genesets.pdf"), width = 18, height = len * 0.15 + 2)
}
...
pathways <- data.frame(
  ensg = c(ISG_core, Cholesterol),          # ENSG (gene names)
  pathway = c(rep("ISG_Core", length(ISG_core)),  # Assign "ISG_Core" to the first set
              rep("Cholesterol", length(Cholesterol))) # Assign "Cholesterol" to the second set
)

data <- merge(limmaRes, pathways, by ="ensg")
# Plotting the dot plot
f1_3<-ggplot(data%>% filter(coef %in% coefficients),
             aes(x = coef, y = ensg,
         color = logFC,
  ...
  scale_size_continuous(
    range = c(2, 6),  # Set the actual size range from 2 to 30
    breaks = c(1,2,4,5,30)  # Set specific breaks to create distinct point sizes
  ) +
  labs(title = "Differentially expressed genesets:Cholesterol biosynthesis",
       ...
       color = "logFC",
       size = "-log10(adj.P.Val)") +
  facet_grid(cols = vars(celltype),rows = vars(pathway),scales = "free_y", space = "free") +
