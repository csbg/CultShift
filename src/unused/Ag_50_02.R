...
base.dir <- "Ag_SCRNA_50_02_ProjectionTrajectories_proj/"
out <- dirout(base.dir)
source("src/FUNC_ProjecTILs_PLUS_Ar.R")
...
mobjs <- list()
for(tissuex in c("ex.vivo","in.vivo")){
...
pseudotime <- fread(dirout_load("SCRNA_50_01_Trajectories")("Values_in.vivo.tsv"))
...
for(sx in unique(query$sample)){
  ...
  tryCatch({
    ...
    print(paste("Error at item 1359:", e))
    # Investigate the inconsistent columns in the list l
    print(str(l))  # Structure of the list
    print(ncol(l[[1]]))  # Check columns in item 1
    print(ncol(l[[1359]]))  # Check columns in item 1359
  })
  trace(ref.trajectory.predict, quote(print(str(l))))
  ??ref.trajectory.predict
...
readRDS(dirout_load("SCRNA_10_collect_UMAPs")("ProjVivo_celltypes.RDS"))
