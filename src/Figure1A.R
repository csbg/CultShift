...
gene_counts <- gene_counts %>%
  mutate(x_val = ifelse(Regulation == "downregulation ex vivo", -log10(count + 1), log10(count + 1)))

# Plot
Fig1.3 <- ggplot(gene_counts, aes(
  y = celltype,
  x = x_val,
  fill = Regulation
)) +
  ...
  scale_fill_manual(
    values = c(
      "downregulation ex vivo" = "#4C889C",
      "upregulation ex vivo" = "#D0154E"
    )
  ) +
  labs(
    y = "Cell type",
...
# Save
ggsave(outdir("Fig1.3.pdf"), plot = Fig1.3, width = 6, height = 5, units = "cm")
################################################################################
#FGSEA
...
############################
#Myc
limmaRes_NTC %>% filter(genes %in% c("Myc","Mycl", "Mycn")) %>%
  ggplot(aes(x= celltype,y=genes))+
