...
write.tsv(
  merge(
    ...
    data.table(ref@reductions$PCA@cell.embeddings, keep.rownames = TRUE),
    by="rn"
...
for(tx in setdiff(PATHS$SCRNA$MONOCLE.NAMES, "in.vivo")){
  ...
  sx <- grep("9d",unique(query$sample))[1]
  for(sx in grep("9d",unique(query$sample),value=T)){
  ...
  head(ref)
    ...
    write.tsv(
        data.table(proj@reductions$PCA@cell.embeddings, keep.rownames = TRUE), out("Output_", tx, "_",sx,"_PCA.tsv"))
  }
}
