...
require(umap)
require(ggrepel)
inp <- dirout("/Ag_ScRNA_08_Pseudobulk_limma_guide")
base.dir <- "Ag_ScRNA_01_Trajectories_comparison/"
out <- dirout(base.dir)
...
# Define tissue names and corresponding RDS file paths
tissues <- c("exvivo", "invivo")
file_paths <- c(inp("exvivo_monocle_proj.rds"), inp("invivo_monocle_proj.rds"))

# Initialize a named list to store Monocle objects
mobjs <- setNames(lapply(file_paths, readRDS), tissues)

# Output the result
cat("Loaded Monocle objects for tissues:", paste(names(mobjs), collapse = ", "), "\n")



# singleR cell types ------------------------------------------------------
#singleR.cell.types <- readRDS(dirout_load("SCRNA_06_02_MergeMarkers")("CellTypes_in.vivo.RDS"))
pseudotime <- fread(dirout_load("Ag_ScRNA_01_Trajectories_comparison")("Values.tsv"))
...
str(ref)
tx <- "ex.vivo"
setdiff(PATHS$SCRNA$MONOCLE.NAMES, "in.vivo")
for(tx in setdiff(PATHS$SCRNA$MONOCLE.NAMES, "in.vivo")){
  
  query <- as.Seurat.NF(mobjs[[tx]])
  unique(query$sample)[grepl("9d",unique(query$sample))][1]
  sx <- unique(query$sample)[grepl("9d",unique(query$sample))][1]
  for(sx in unique(query$sample)[grepl("9d",unique(query$sample))]){
...
ff
pDT <- rbindlist(lapply(ff, function(fx) fread(out(fx))), idcol = "celltype",fill = TRUE)
pDT[, traj := pseudotime.cds.]
pDT$pseudotime.cds. <- NULL
write.tsv(pDT, out("Values.tsv"))
pDT <- merge(pDT, ann_in[Clusters != "HSC"], by="rn")[!is.na(mixscape_class.global)]
...
colnames(ann)[20]<-"samp"

ff <- list.files(out(""), pattern="Output.*.tsv")
ff <- ff[!grepl("in.vivo", ff)]
names(ff) <- gsub("Output_(.+).tsv", "\\1", ff)

pDT <- rbindlist(lapply(ff, function(fx) fread(out(fx))), idcol = "sample")
pDT[, traj := pseudotime]
pDT$pseudotime <- NULL
pDT[, celltype := ct]
write.tsv(pDT, out("Values_ex_leuk.tsv"))
pDT <- merge(pDT, ann, by="rn")[!is.na(mixscape_class.global)]
head(pDT)

invivo<-list.files(out(""), pattern="Output.*.tsv")
invivo<-invivo[grepl("in.vivo", invivo)]
names(invivo) <- gsub("Output_(.+).tsv", "\\1", invivo)

pDT_invivo <- fread(out(invivo))
pDT_invivo[, traj := pseudotime]
pDT_invivo$pseudotime <- NULL

pDT_invivo[, celltype := pseudotime.celltype]
pDT_invivo$pseudotime.celltype <- NULL
colnames(pDT_invivo)<-gsub("UMAP_","UMAP",colnames(pDT_invivo))
head(pDT_invivo)

head(pDT)
?rbind
pDT<-rbind(pDT,pDT_invivo,fill=T)
head(pDT)
#invivo
