...
basedir <- dirout("JAK_STAT_splenic_M_T")
...
limmaRes$coef <- gsub("treatmentex_vivo","ex-vivo",
                          limmaRes$coef)
...
limmaRes_WT <- limmaRes %>%
  filter(coef == "ex-vivo")
limmaRes_WT <- limmaRes_WT %>%
  mutate(coef = paste0(coef,cell_type))
#ranklist
coef_names <- unique(limmaRes_WT$coef)


# Directory for rank lists
dir.create(basedir("Ranklists"), showWarnings = FALSE)
...
ranklists <- map(coef_names, function(coefx) {
  ...
  saveRDS(
    ranks,
    file = file.path(basedir("Ranklists"), paste0(coefx, "_logFC_ranklist.rds"))
  )
  
  # Save as TXT (universal)
  write.table(
    data.frame(gene = names(ranks), logFC = ranks),
    file = file.path(basedir("Ranklists"), paste0(coefx, "_logFC_ranklist.txt")),
    sep = "\t",
    quote = FALSE,
    row.names = FALSE
  )
...
##########################
#Saving_rds
dataVoom_T8_WT <- dataVoom_T8[,colnames(dataVoom_T8)[grepl("WTN",colnames(dataVoom_T8))]] 
dataVoom_M_WT <- dataVoom_M[,colnames(dataVoom_M)[grepl("WTN",colnames(dataVoom_M))]]
...
longer_dataVoom_M <- dataVoom_M_WT %>%
  as.data.frame() %>%
  ...
  inner_join(meta_WT, by ="sample")%>%
  mutate(tissue = ifelse(tissue == "ex_vivo","ex.vivo","in.vivo"))
longer_dataVoom_M$tissue <- factor(longer_dataVoom_M$tissue, 
                                 levels = c("in.vivo" ,"ex.vivo"))

longer_dataVoom_M <- longer_dataVoom_M %>%
...
longer_dataVoom_M %>%
  write_rds(basedir("Spleen_M.rds"))
#T8
longer_dataVoom_T8 <- dataVoom_T8 %>%
  as.data.frame() %>%
  rownames_to_column("genes") %>%
  as_tibble() %>%
  pivot_longer(
  ...
  inner_join(meta_WT, by ="sample")%>%
  mutate(tissue = ifelse(tissue == "ex_vivo","ex.vivo","in.vivo"))
longer_dataVoom_T8$tissue <- factor(longer_dataVoom_T8$tissue, 
                                    levels = c("in.vivo" ,"ex.vivo"))

longer_dataVoom_T8 <- longer_dataVoom_T8 %>%
  dplyr::select(c("genes",
                  "Expression",
                  "sample",
                  "tissue"))
longer_dataVoom_T8 %>%
  write_rds(basedir("Spleen_T8.rds"))
