...
for (ex in c("7d","9d")){
  for (inv in c("14d","28d")){
    exvivo = fread(dirout(paste0(inp, "ex.vivo_",ex,"_useClusters"))("DEG_Results_all.tsv")) %>% 
      select(c("guide","gene_id","estimate","p_value"))#%>% filter(guide %in% gene_list)
    invivo = fread(dirout(paste0(inp, "in.vivo_",inv,"_useClusters"))("DEG_Results_all.tsv")) %>% 
      select(c("guide","gene_id","estimate","p_value"))#%>% filter(guide %in%head(exvivo)head(exvivo)head(exvivo)head(exvivo)head(exvivo)head(exvivo)head(exvivo)head(exvivo) gene_list)
    head(exvivo)
    ggplot(exvivo, aes(x=estimate, y=-log10(p_value))) + 
      geom_point()+
      theme_bw(12) +
      geom_text_repel(aes(label=gene_id))+facet_grid(~guide)
    ggsave(out)
    ...
    for (corr in c("pearson","spearman")){
      ...
      mine.heatmap <- ggplot(data = diag_correlation, mapping = aes(x = "correlation",
         ...
         fill = correlation)) + geom_tile() + xlab(label = "Sample")+theme(axis.text=element_text(size=14)) 
      mine.heatmap
     
      ggsave(out_new("pheatmap_logFC_corresponding_ko","invivo",inv,"exvivo",ex,".png"), w=15,h=15)
