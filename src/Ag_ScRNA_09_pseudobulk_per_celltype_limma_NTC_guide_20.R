...
basedir<-dirout("Ag_ScRNA_09_pseudobulk_per_celltype_limma_NTC_guide_20")
...
ggsave(basedir("logFC_distribution_per_celltype_TMM_threshold_15.pdf"),w=20)

# Install ggplot2 if you don't have it
# install.packages("ggplot2")


# Assuming limmaRes is your data frame containing p-values in 'P.Value' column
ggplot(ex_in_NTC_per_ct, aes(x = P.Value)) +
  geom_histogram(bins = 50, color = "black", fill = "blue") +
  labs(title = "P-value Distribution", x = "P-value", y = "Frequency") +
  theme_minimal()

ggsave(basedir("pvalue_distribution_per_celltype_TMM_threshold_20.pdf"),w=20)
