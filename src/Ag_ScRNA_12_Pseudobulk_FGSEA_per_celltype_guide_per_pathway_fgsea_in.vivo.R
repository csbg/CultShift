...
ref_gene_sets <- limmaRes_NTC %>%
  filter(group != "n.s") %>%
  group_by(celltype) %>%
  summarise(ref_genes = list(unique(genes)), .groups = "drop")


input_gene_sets <- limmaRes %>%
...
run_fisher <- function(sig, ref, bg) {
  ...
  if (length(sig) == 0 || length(ref) == 0 || length(bg) == 0) {
    return(data.frame(overlap = NA, p.value = NA, odds.ratio = NA))
  }
...
ggplot(enrichment_results, aes(x = coef, y = celltype)) +
  geom_point(aes(color = odds.ratio, size = pmin(5,-log10(p.value)))) +
  scale_color_gradient2(
    low = "#4C889C",
    mid = "white",
    high = "#D0154E")+
  labs(
    title = "Enrichment of coef-celltype DE genes vs NTC celltype signature",
    x = "Coefficient",
    y = "Cell Type",
...
gsea.res <- run_gsea(limmaRes, enr.terms, celltypes = unique(limmaRes$celltype),
                     coefs =unique(limmaRes$coef))
gsea.res %>% write_rds(basedir("fgsea_ex.vivo_vs_in.vivo_per_CT_interaction_invivo.rds"))
...
gsea.res <- read_rds(basedir("fgsea_ex.vivo_vs_in.vivo_per_CT_interaction_invivo.rds"))
pathways <- c(
...
# ggplot(enrichment_results, aes(x = coef, y = celltype)) +
#   # Size by overlap count (the number of overlapping genes)
#   geom_point(aes(size = pmin(2,-log10(fisher_p_adj)), color = perc.overlap)) +
#   
#   # Add text labels for DEG counts or overlap (optional)
#   # geom_text(
#   #   data = filter(enrichment_results, !is.na(overlap)), 
#   #   aes(label = overlap), vjust = -1.1, size = 3.5, color = "black"
#   # ) +
#   
...
#   
#   labs(
#     title = "Enrichment Bubble Plot with DEG Counts, Overlap & Percentage Overlap",
#     subtitle = "Size = Overlap | Color = Percentage Overlap | Label = Overlap Count",
#     x = "Coefficient",
#     y = "Cell Type"
#   ) +
#   optimized_theme_fig()
# 
# ggsave(basedir("enrichment_to_ntc_dot.pdf"), w=12,h=8, units = "cm")
# # Create a combined key to simplify matching
# 
# # Join with enrichment results
# # Combine the DEG counts with the enrichment data
# combined_results <- enrichment_marked %>%
#   left_join(data, by = c("coef", "celltype")) %>%
#   mutate(
#     # Create any derived columns if needed, e.g., log10 of Count if needed for normalization
#     log10_count = log10(Count + 1) # +1 to avoid log(0)
#   )
# 
# 
# ggplot(combined_results, aes(x = coef, y = celltype)) +
#   # Size by overlap count (previous)
#   geom_point(aes(size = overlap, color = log10_count)) +
#   
#   # Add circle outlines for the "marked" combos
#   geom_point(
#     data = filter(combined_results, !is.na(Count)),
#     shape = 21, fill = NA, color = "black", size = 6, stroke = 1
#   ) +
#   
#   # Customize the color and size scales
#   scale_color_gradient(low = "lightblue", high = "darkred", name = "log10(DEG Count)") +
#   scale_size(range = c(2, 10), name = "Overlap") +
#   
#   labs(
#     title = "Enrichment Bubble Plot with DEG Counts",
#     subtitle = "Size = overlap | Color = log10(DEG Count) | Circle = Marked in Data",
#     x = "Coefficient",
#     y = "Cell Type"
#   ) +
...
# 
# ggsave(basedir("enrichment_to_ntc_dot_highlight.pdf"))
